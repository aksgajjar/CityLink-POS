...
def premium_dialog_qss() -> str:
    ...
    Includes consistent styling for QSpinBox / QDateEdit / QCheckBox so
    macOS native dark-mode chrome doesn't bleed through. Forces Fusion-
    compatible light theme on every dialog.
    """
    c = COLORS
    return (
        f"QDialog {{ background-color: {_ADMIN_BG}; color: {c['text_dark']}; }}"
        f"QFrame#card {{"
        ...
        f"}}"
        # Inputs
        f"QLineEdit, QComboBox, QSpinBox, QDoubleSpinBox, QDateEdit, QTimeEdit, QDateTimeEdit {{"
        f"  background: white; color: {c['text_dark']};"
        f"  border: 1px solid #B0BEC5; border-radius: 6px;"
        f"  padding: 8px 12px; min-height: 22px; font-size: 12pt;"
        ...
        f"  border: 2px solid {c['blue_mid']};"
        f"}}"
        f"QLineEdit:disabled, QComboBox:disabled, QSpinBox:disabled,"
        f" QDateEdit:disabled {{"
        f"  background: #ECEFF1; color: #8A8F95;"
        f"}}"
        # SpinBox arrows
        f"QSpinBox::up-button, QDoubleSpinBox::up-button,"
        f" QDateEdit::up-button, QTimeEdit::up-button, QDateTimeEdit::up-button {{"
        f"  subcontrol-origin: border; subcontrol-position: top right;"
        f"  width: 22px; border-left: 1px solid {_ADMIN_BORDER};"
        f"  border-bottom: 1px solid {_ADMIN_BORDER};"
        f"  background: white;"
        f"}}"
        f"QSpinBox::down-button, QDoubleSpinBox::down-button,"
        f" QDateEdit::down-button, QTimeEdit::down-button, QDateTimeEdit::down-button {{"
        f"  subcontrol-origin: border; subcontrol-position: bottom right;"
        f"  width: 22px; border-left: 1px solid {_ADMIN_BORDER};"
        f"  background: white;"
        f"}}"
        f"QSpinBox::up-arrow, QDoubleSpinBox::up-arrow,"
        f" QDateEdit::up-arrow, QTimeEdit::up-arrow {{"
        f"  width: 8px; height: 8px;"
        f"}}"
        # ComboBox dropdown
        f"QComboBox::drop-down {{"
        f"  subcontrol-origin: padding; subcontrol-position: top right;"
        f"  width: 26px; border-left: 1px solid {_ADMIN_BORDER};"
        f"  background: white;"
        f"}}"
        f"QComboBox QAbstractItemView {{"
        f"  background: white; color: {c['text_dark']};"
        f"  border: 1px solid {_ADMIN_BORDER}; outline: 0;"
        f"  selection-background-color: {c['blue_mid']};"
        f"  selection-color: white;"
        f"}}"
        # Checkboxes
        f"QCheckBox {{ font-size: 11pt; spacing: 8px;"
        f"  background: transparent; color: {c['text_dark']}; }}"
        f"QCheckBox::indicator {{ width: 20px; height: 20px;"
        f"  border: 1px solid #B0BEC5; border-radius: 4px;"
        f"  background: white; }}"
        f"QCheckBox::indicator:checked {{"
        f"  background: {c['blue_mid']}; border: 1px solid {c['blue_mid']};"
        f"  image: none;"
        f"}}"
        f"QCheckBox::indicator:hover {{ border: 1px solid {c['blue_mid']}; }}"
        # Calendar popup (date edit)
        f"QCalendarWidget QWidget {{ background: white; color: {c['text_dark']}; }}"
